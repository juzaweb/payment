...
<body class="body--custom-background-color ">
        ...
        <div class="wrap">
            <h2>
                ...
                <label class="control-label hidden-small-device">
                    ({{ $cart->items->count() }} {{ trans('payment::translation.products') }})
                </label>
    ...
    <form method="post" action="{{ route('payment.checkout', [$module]) }}" data-toggle="validator"
        ...
        <div class="wrap" context="checkout"
            define='{checkout: new Juzaweb.StoreCheckout(this,{ token: "{{ $cart->id }}", email: "{{ $user?->email }}", totalOrderItemPrice: "${{ $cart->items->sum(fn($item) => (float) ($item->orderable?->price ?? 0) * $item->quantity) }}", shippingFee: 0, freeShipping: false, requiresShipping: false, existCode: false, code: "", discount: 0, settingLanguage: "vi", moneyFormat: "", discountLabel: "{{ trans('payment::translation.free') }}", districtPolicy: "optional", wardPolicy: "hidden", district: "", ward: "", province:"", otherAddress: false, shippingId: 0, shippingMethods: {}, customerAddressId: 0, reductionCode: "", stripePublishKey: "{{ $stripePublishKey }}" }), email: "{{ $user?->email }}", payment_method_id: "{{ $paymentMethods[0]->driver ?? '' }}"}'>
            <div class='sidebar '>
                <div class="sidebar_header">
                    <h2>
                        <label class="control-label">{{ trans('payment::translation.order') }}
                            ({{ $cart->items->count() }} {{ trans('payment::translation.products') }})</label>
                    </h2>
                    <hr class="full_width" />
                </div>
                    ...
                    <div class="order-summary order-summary--discount">
                        <div class="summary-section">
                                ...
                                <div class="field__input-btn-wrapper">
                                    <div class="field__input-wrapper">
                                        <input bind="code" name="code" type="text"
                                            ...
                                            placeholder="{{ trans('payment::translation.discount_code') }}"
                                            value="" id="checkout_reduction_code" />
                                    </div>
                                    <button bind-event-click="reduction_code = false, caculateShippingFee('')"
                                        class="btn btn-primary event-voucher-apply"
                                        type="button">{{ trans('payment::translation.apply') }}</button>
                                </div>
                            </div>
                            ...
                            <div bind-class="{'warning' : existCode && !freeShipping && discount == 0,'success' : existCode && ( freeShipping || discount >0 )}"
                                ...
                                <div bind="(discountShipping && freeShipping) ? '{{ trans('payment::translation.free') }}' : discount"
                                    class="pull-right">
                                    0
                                </div>
                                <input bind-event-click="removeCode('')" class="btn btn-delete" type="button"
                                    value="×" name="commit">
                            </div>
                            <div class="error mt10 hide" bind-show="inValidCode">
                                {{ trans('payment::translation.discount_code_is_not_valid') }}
                            </div>
                    ...
                    <div class="form-group clearfix hidden-sm hidden-xs">
                        <div class="field__input-btn-wrapper mt10">
                            ...
                            <input class="btn btn-primary btn-checkout"
                                ...
                                value="{{ strtoupper(trans('payment::translation.order')) }}" />
                        </div>
                    </div>
                ...
                <div class="main_content">
                    <div class="row">
                        <div class="col-md-6 col-lg-6">
                            <div class="section"
                                define="{billing_address: {full_name: '{{ $user?->name }}', phone: '{{ $user?->phone }}'}}">
                                    ...
                                    <div class="layout-flex layout-flex--wrap">
                                        <h2 class="section__title layout-flex__item layout-flex__item--stretch">
                                            <i class="fa fa-id-card-o fa-lg section__title--icon hidden-md hidden-lg"
                                                aria-hidden="true"></i>
                                            <label
                                                class="control-label">{{ trans('payment::translation.information') }}</label>
                                        </h2>
                                            ...
                                            <div class="form-group">
                                                <div class="field__input-wrapper"
                                                    ...
                                                    <input name="name" type="text"
                                                        ...
                                                        value="{{ $user?->name }}" @auth disabled @endauth />
                                                </div>
                                            ...
                                            <div class="form-group">
                                                <div class="field__input-wrapper"
                                                    ...
                                                    <input name="phone" bind-event-change="saveAbandoned()"
                                                        ...
                                                        data-error="{{ trans('payment::translation.please_enter_full_phone') }}"
                                                        pattern="^([0-9,\+,\-,\(,\),\.]{8,20})$"
                                    ...
                                    <div class="form-group m0">
                                        <div>
                                            <label class="field__input-wrapper" bind-class="{'js-is-filled': note}"
                                                ...
                                                <textarea name="notes" bind-event-change="saveAbandoned()" bind-event-focus="handleFocus(this)"
                                                    bind-event-blur="handleFieldBlur(this)" bind="note" class="field__input form-control m0"></textarea>
                                            </label>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-md-6 col-lg-6">
                            ...
                            <div class="section hidden-md hidden-lg">
                                <div class="form-group clearfix m0">
                                    <input class="btn btn-primary btn-checkout" data-loading-text="Đang xử lý"
                                        type="button"
                                        bind-event-click="paymentCheckout('AIzaSyAjQYbV19v7UMDVk0cDZ54yKh3OP1hQhbk;AIzaSyCLd-YkfOzBXlNGfS_FNLnpolyME1tRAJI;AIzaSyDdvilzaJlb50t2IRC3PrfSb1lNzf6n3pQ')"
                                        value="{{ trans('payment::translation.order') }}" />
                                </div>
